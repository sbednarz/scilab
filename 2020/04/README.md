...
Conversion 𝛼 of the monomer into the polymer in the reactor (R) is 30% per pass. Unreacted monomer is seperated and mixed (in the mixer, M) with a fresh monomer stream F<sub>M</sub>. Calculate flow of the polymer (F<sub>P</sub>) and the recycled
stream (F<sub>R</sub>) assuming ideal separation of the polymer in the separator (S). Hint: combine in two equation system, total mass balance, and the monomer or the polymer balance in the seperator.
